...
async function fetchAll() {
  ...
  const push = (list) => {
    for (const m of list) {
      if (m.title && m.backdrop_path && !seen.has(m.id)) {
        seen.add(m.id);
        movies.push(m);
      }
    }
  ...
  // Top rated pages 11–30
  console.log('  Fetching top-rated pages 11–30…');
  for (let p = 11; p <= 30; p++) push(await tmdbGet(`/movie/top_rated?page=${p}`));

  // Discover by decade
  const decades = [
    ['2020', '2025'], ['2015', '2019'], ['2010', '2014'],
    ['2005', '2009'], ['2000', '2004'], ['1990', '1999'],
    ['1980', '1989'], ['1970', '1979'],
  ];
  for (const [from, to] of decades) {
    console.log(`  Fetching discover ${from}–${to}…`);
    for (let p = 1; p <= 5; p++) {
      push(await tmdbGet(`/discover/movie?sort_by=popularity.desc&primary_release_date.gte=${from}-01-01&primary_release_date.lte=${to}-12-31&page=${p}`));
    }
  }

  // Trending all time pages 6–15
  console.log('  Fetching trending pages 6–15…');
  for (let p = 6; p <= 15; p++) push(await tmdbGet(`/trending/movie/week?page=${p}`));

  return movies;
}

async function main() {
  console.log('\n🎬  WUNWURD extra seed starting…\n');
